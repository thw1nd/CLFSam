...
from Fusion_Blockv2 import MultiScaleFreqSEFusionV3
...
class FreASam(nn.Module):
    def __init__(self,
        ...
        self.encoder = Hiera(
            ...
            v_lora_alpha=self.v_lora_alpha,
            drop_path_rate=0.05) # if you want change params, please change them in image encoder2.py

        self.fusion_module = MultiScaleFreqSEFusionV3(
    ...
    def forward(self, rgb, aux1, aux2):
        h, w = rgb.size()[-2:]

        x_list, aux1_list, aux2_list = self.encoder(rgb, aux1, aux2)
        # fuse4x, fuse8x, fuse16x, fuse32x = self.fusion_module(x_list, aux1_list, aux2_list)

        fuse_list = self.fusion_module(x_list, aux1_list, aux2_list)
...
def test_model_performance(model, device='cuda'):
    ...
    aux2_input = torch.randn(1, 3, 512, 512).to(device)

    try:
        flops, params = profile(model, inputs=(rgb_input, sar_input,aux2_input), verbose=False)
...
def test_fps_comprehensive(model, device='cuda', num_warmup=100, num_test=500):
    ...
    aux2 = torch.randn(1, 3, 512, 512).to(device)
...
if __name__ == "__main__":
    ...
    x1 = torch.randn(1, 3, 512, 512).to(device)
