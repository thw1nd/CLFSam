...
def apply_unfreeze_from_config(model, epoch, cfg, logger=None):
    ...
    def is_proj(name: str) -> bool:
        return (".proj." in name) or (".aux1_proj." in name)

    part_pred = {"norm": is_norm, "mlp":  is_mlp, "attn": is_attn, "proj": is_proj}
...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x, aux1):
        return self.net(x, aux1)
    ...
    def training_step(self, batch, batch_idx):
        img, aux1, mask = batch['img'], batch['aux2'], batch['gt_semantic_seg']

        # ------- 前向 -------
        prediction = self.net(img, aux1)
        ...
        lambda_fuse = _cfg_get(self.config, 'fusion_aux_lambda', 0.1)
    ...
    def validation_step(self, batch, batch_idx):
        img, aux1, mask = batch['img'], batch['aux2'], batch['gt_semantic_seg']
        prediction = self.forward(img, aux1)
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_last=_cfg_get(config, 'save_last', False),
